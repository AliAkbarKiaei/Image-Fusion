...
for sourceNumber=1:length(sourceImages)
    ...
    clc
    disp(['Del_Image_Fusion_weighted: ', sourceImage]);
    %%
    close all;
    ...
    U_fused = uint8(fn_fusion_GV_weighted_2(U1, ROI_BW, ROI_U2, pos));   % cloning image by Perez method
    imwrite( U_fused , strcat(cd,'/images/',sourceImage,'_fused_Del_weighted.png'));
end
