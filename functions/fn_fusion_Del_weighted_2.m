function weightedFusionDel = fn_fusion_Del_weighted_2(U1, ROI_BW, ROI_U2, pos)
...
for channel = 1:hU
    Gxx_ROI_U(:,:,channel) = conv2(Gx_ROI_U(:,:,channel),filter(end:-1:1),'same')/sum(abs(filter));
    Gyy_ROI_U(:,:,channel) = conv2(Gy_ROI_U(:,:,channel),filter(end:-1:1)','same')/sum(abs(filter));
end
    ...
    for r=3:rROI-2
        for c=3:cROI-2
            if ROI_BW(r,c)==1
                 ...
                 if ROI_BW(r-1,c)==0,  ROI_temp(r-2,c,:)=ROI_temp(r-1,c,:); Gy_ROI_U(r-1,c,:)=Gy_ROI_U(r,c,:); end
                 if ROI_BW(r+1,c)==0,  ROI_temp(r+2,c,:)=ROI_temp(r+1,c,:); Gy_ROI_U(r+1,c,:)=Gy_ROI_U(r,c,:); end
                 if ROI_BW(r,c-1)==0,  ROI_temp(r,c-2,:)=ROI_temp(r,c-1,:); Gx_ROI_U(r,c-1,:)=Gx_ROI_U(r,c,:); end
                 if ROI_BW(r,c+1)==0,  ROI_temp(r,c+2,:)=ROI_temp(r,c+1,:); Gx_ROI_U(r,c+1,:)=Gx_ROI_U(r,c,:); end
                
                ROI_temp(r,c,:) = 0.25*(  ROI_temp(r-2,c,:)+ROI_temp(r+2,c,:)+ROI_temp(r,c-2,:)+ROI_temp(r,c+2,:)  ) ...% - Laplace_ROI_U(r,c,:) ;
                    - 0.5*(  Gx_ROI_U(r,c+1,:) - Gx_ROI_U(r,c-1,:) + Gy_ROI_U(r+1,c,:) - Gy_ROI_U(r-1,c,:)  );
